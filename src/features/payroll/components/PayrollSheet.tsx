...
export function PayrollSheet({
  ...
  const handleDeleteEntry = async (entry: PayrollEntry) => {
    const confirmed = await confirm({
      title: "Remove from payroll sheet?",
      message: `Remove ${entry.employee.name}'s entry for ${entry.employee.periodStart} – ${entry.employee.periodEnd}? This does not delete payslip history.`,
      confirmLabel: "Remove",
      confirmColor: "error",
    })
    if (confirmed) {
  ...
  return (
    ...
    >
      ...
      {entries.length === 0 ? (
        <EmptyState
          icon={<PeopleIcon />}
          title="Payroll sheet is empty"
          description='Fill out the employee form, review the pay summary, then click "Add to Sheet" to build your payroll register for this run.'
        />
      ) : (
        <Stack spacing={2}>
