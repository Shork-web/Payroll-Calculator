...
export function EmployeeRecordsViewer({ entries, onEdit, onDelete }: EmployeeRecordsViewerProps) {
  ...
  const handleExportPayslip = () => {
    ...
    void (async () => {
      ...
      exportBulkPayslipsPdf([selectedEntry], getPdfPaperSize())
    })()
  }
  ...
  const handleExportComputation = () => {
    ...
    void (async () => {
      ...
      const { getPdfPaperSize } = await import("@/lib/exports/pdfShared")
      exportBulkComputationsPdf([selectedEntry], getPdfPaperSize())
    })()
  }
  ...
  const handleExportAttendance = () => {
    if (!selectedEntry) return
    void (async () => {
      const { exportAttendanceCertificatePdf } = await import("@/features/records/lib/exports/attendanceCertificatePdf")
  ...
  const handleDeleteEntry = async (entry: PayrollEntry) => {
    const confirmed = await confirm({
      ...
      message: `Permanently delete ${entry.employee.name}'s record for ${entry.employee.periodStart} – ${entry.employee.periodEnd}? This cannot be undone.`,
      confirmLabel: "Delete",
      confirmColor: "error",
    })
    if (confirmed) {
  ...
  if (entries.length === 0) {
    return (
      <EmptyState
        ...
        description='Compute payroll in the Calculator tab and click "Add to Sheet" to build payslip history here.'
      />
    )
  }
              ...
              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 2, mb: 2 }}>
                ...
                <Stack direction="row" spacing={1}>
                  ...
                  <Tooltip title="Delete this record">
                    <IconButton size="small" onClick={() => void handleDeleteEntry(selectedEntry)} sx={{ color: "error.main", border: 1, borderColor: "divider" }}>
